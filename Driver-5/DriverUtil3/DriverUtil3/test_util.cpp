	...
	PPS_CREATE_NOTIFY_INFO CreateInfo)
{
	...
	//对进程内存或者结束进程，需要一个WorkItem或者一个Thread
	//
}
class thread_mon
{
public:
	...
	VOID thread_create_routine(
		...
		BOOLEAN Create)
	{
		//TODO:think 怎么拒绝线程创建 对线程的USER_FRAME或者APC进行处理
		hello();
		if (Create)
		{
	...
	PVOID Arg2)
{
	...
	case RegNtSetValueKey:
		//
	{
		//Kernelmode ValueName
		//UserMode ValueName-->地址是user地址，api写上去的，如果用ZwXXX函数就是直接的那个地址
...
ddk::nt_process_callback m_process_callback;
ddk::nt_thread_callback m_thread_callback;
ddk::nt_image_callback m_image_callback;
ddk::nt_regcmp_callback m_regcmp_callback;
ddk::nt_object_callback m_object_callback;
void test_util(PUNICODE_STRING RegistryPath)
{
	m_process_callback.getInstance()->reg_callback_ex(std::bind(&ProcessNotifyRoutine, 0x1111,
		std::placeholders::_1,
		std::placeholders::_2,
		std::placeholders::_3));
	m_thread_callback.getInstance()->reg_callback(std::bind(&thread_mon::thread_create_routine,
		&threadA,
		std::placeholders::_1,
		std::placeholders::_2,
		std::placeholders::_3));
	m_image_callback.getInstance()->reg_callback(LoadImageNotifyRoutine);
	m_regcmp_callback.getInstance()->reg_callback(cmp_callback);
	...
	m_object_callback.getInstance()->set_process_pre_callback(ProcessPre);
