...
FnparseGetFunctionLength(UCHAR *FunctionAddress) {
  ...
  for (auto address = FunctionAddress;; address = base + end) {
    auto entry = FnparseLookupFunctionEntry(
        ddk::mem_util::UtilDataToFp(reinterpret_cast<UCHAR *>(address)),
...
FnparseGetEpilogueInfo(UCHAR *FunctionAddress) {
  ...
  UCHAR *base = nullptr;
  auto entry = FnparseLookupFunctionEntry(
      ddk::mem_util::UtilDataToFp(reinterpret_cast<UCHAR *>(FunctionAddress)),
      reinterpret_cast<void **>(&base));
  if (!entry) {
    return {};
  }
  ...
  auto epilogueAddress = reinterpret_cast<UCHAR *>(
      ddk::mem_util::MmMemMem(FunctionAddress, length, epilogueBytesNaked, epilogueSize));
  if (!epilogueAddress) {
    return {};
  }
