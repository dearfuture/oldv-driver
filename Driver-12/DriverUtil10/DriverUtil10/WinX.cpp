    ...
    __in const WinXSymbols& Symbols)
{
	...
	auto exit_ = std::experimental::make_scope_exit([&](){KeLowerIrql(old);
	lock.unlock(); });
    ...
    {
        // For 7 and Vista and win8 win8.1 win10
		{
			WinXpEnumBigPages(WinXpCollectPatchGuardContexts, &contexts, 0,
				MIN_SIZE_FILTER,
				*g_WinXSymbols.PoolBigPageTable,
    ...
    auto status = STATUS_OBJECT_PATH_NOT_FOUND;
	...
	if (NT_SUCCESS(status))
	{
		...
		for (SIZE_T i = 0; i < contexts.NumberOfPgContexts; ++i)
		{
			contexts.PgContexts[i].phyAddr = MmGetPhysicalAddress(PVOID(contexts.PgContexts[i].PgContext));
			DBG_PRINT("PgContext phyaddr %p Key %016llX\r\n", PVOID(contexts.PgContexts[i].phyAddr.QuadPart), contexts.PgContexts[i].XorKey);
		}
	}
    return status;
}
    ...
    __in SIZE_T NumberOfBigPageTable)
{
    for (SIZE_T i = 0; i < NumberOfBigPageTable; ++i)
    {
		...
		//if (ddk::util::IsWindows80()|| ddk::util::IsWindows81())
		//{
		//	auto special = reinterpret_cast<POOL_TRACKER_BIG_PAGES*>(entry);
		////	LOG_DEBUG("VA %p Type %x\r\n", special->Va,special->PoolType);
		//	if ((special->PoolType&PagedPool)!=0)
		//	{
		//		continue;
		//	}
		//}
		...
		//{
		//	auto special = reinterpret_cast<ddk::ntos_space::win10_14393_x64::POOL_TRACKER_BIG_PAGES*>(entry);
		////	LOG_DEBUG("VA %p Type %x\r\n", special->Va, special->PoolType);
		//	if ((special->PoolType&PagedPool) != 0)
		//	{
		//		continue;
    ...
    __inout_opt void* Context)
{
	...
	{
		if (StartAddress<0xFFFFD00000000000UI64)
		{
			return true;
		}
		//if (!MmIsNonPagedSystemAddressValid(PVOID(StartAddress)))
		//{
		//	return true;
		//}
		//PVOID base = nullptr;
		//RtlPcToFileHeader(PVOID(StartAddress), &base);
		//if (base)
		//{
		//	return true;
		//}
    ...
    for (SIZE_T searchedBytes = 0; searchedBytes < SizeInBytes; /**/)
    {
        ...
        if (result.PgContext)
        {
			...
			LOG_DEBUG("[%5Iu:%5Iu] PatchGuard %016llX : XorKey %016llX\n",
    ...
    __in PatchGuardContextInfo& Info)
{
	...
	if (ddk::util::IsWindows10())
	{
		auto pgContext10 = reinterpret_cast<PgContextBase10*>(Info.PgContext);
		// Install hook
		static const auto HEADER_SIZE =
			FIELD_OFFSET(PgContextBase10, unknown2);
		const auto searchSizeInBytes =
			pgContext10->ContextSizeInQWord * sizeof(ULONG64) + HEADER_SIZE;

		auto pgSdbpCheckDll = ddk::mem_util::MmMemMem(pgContext10, searchSizeInBytes,
		...
		pgContext10->CmpAppendDllSection[WINX_HOOK_OFFSET + 0] = WINX_HOOK_CODE;
		pgContext10->CmpAppendDllSection[WINX_HOOK_OFFSET + 1] = 0xc3;   // RET
		ASSERT(WinXpIsPatchGuardContextPatched(*pgContext));
		return;
	}
    ...
    __in PatchGuardContextInfo& Info)
{
	if (ddk::util::IsWindows10())
	{
		...
		return reinterpret_cast<PgContextBase*>(decryptedPgContext);
	}
    // The first some bytes can simply be XOR-ed
    auto pgContext = reinterpret_cast<ULONG64*>(Info.PgContext);
    static const auto NUMBER_OF_TIMES_TO_DECRYPT =
    ...
    __in PgContextBase* DecryptedPgContext)
{
	if (ddk::util::IsWindows10())
	{
		const auto pgContextSize = (reinterpret_cast<PgContextBase10*>(DecryptedPgContext))->ContextSizeInQWord;
		auto pgContext = reinterpret_cast<ULONG64*>(Info.PgContext);
