...
#include <ntddkbd.h>
#include "pg_enhance.h"
#include "tdi_fw.h"
#include "nt_inject.h"
#include "nt_process_protect.h"
...
//////////////////////////////////////////////////////////////////////////
ddk::CTDI_Server tcpserver;
void onAccept(INT_PTR _Socket)
{
	CHAR szHel[] = "hello world";
	ddk::CTDI_Client cli(_Socket);
	cli.Send(szHel, sizeof(szHel));
	cli.Clear();
}
//////////////////////////////////////////////////////////////////////////
void wskudp()
{
	...
	ddk::CWSK_UdpClient client;
	client.create(33333);
	client.sendto(L"192.168.0.100", L"43333", LPCVOID(GreetMessage), sizeof(GreetMessage) - 1, BufferSize);
	client.shutdown();
}
void wsktcp()
{
	...
	ddk::CWSK_TcpClient client;
	if (!client.connect(L"192.168.0.100", L"23333"))
	{
		DBG_PRINT("connect failed\r\n");
		return;
	}
	
	if (!client.send(GreetMessage, sizeof(GreetMessage) - 1, BufferSize))
	{
...
ddk::CWSK_Server echo_srv;
static volatile LONG	g_ClientsCount = 0;
void echo_thread(PWSK_SOCKET socket)
{
	UCHAR	Buffer[256] = { 0 };
	ULONG	BufferSize = 0;
	CHAR	GreetMessage[] = "Hello there\r\n";
	auto client = ddk::CWSK_TcpClient(socket);
	if (client.send(GreetMessage, sizeof(GreetMessage) - 1, BufferSize))
	{
		do
		{
			BufferSize = 0;
			if (!client.recv(Buffer, sizeof(Buffer), BufferSize))
			{
				break;
			}
			auto sendSize = BufferSize;
			BufferSize = 0;
			if (!client.send(Buffer,sendSize,BufferSize))
			{
				break;
			}
		} while (1);
...
void onAccept_echo_srv(PSOCKADDR_IN RemoteAddress, PWSK_SOCKET socket)
{
	DBG_PRINT("Client From %u.%u.%u.%u:%u\r\n",
		...
		ddk::KSOCKET::htons(RemoteAddress->sin_port));
	InterlockedIncrement(&g_ClientsCount);
	auto client_thread = ddk::CThread(echo_thread, socket);
	client_thread.detach();
}
void wskEchoSrv()
{
	if (!echo_srv.bind(38888))
	{
		DBG_PRINT("bind wrong\r\n");
		return;
	}
	echo_srv.do_worker(onAccept_echo_srv);
...
ddk::inject::nt_hook_inject hookinject;
//////////////////////////////////////////////////////////////////////////
void test_util(PUNICODE_STRING RegistryPath)
{
	ddk::mem_util::init_mem_util();

	ddk::protect::nt_process_protect::getInstance().protect_process(L"testapp.exe");
	
	//神力注入
	hookinject.set_hook_inject(L"c:\\testdll.dll", L"", "testbox.exe");

	//ddk::pg_enhance::getInstance().DisPg();

	//init_tdi_filter();
	//Socket测试
	/*tcpserver.Bind(8090);
	tcpserver.StartWorker(onAccept);*/

	//WSK Test

	//WSK_Test();

	////http 主动下载ntoskrnl的符号
	//ddk::http_response response;
	//ddk::http_client http;
	//{
	//	std::string pdbfilename, symname;
	//	std::string cabfilename;
	//	PVOID imageBase = nullptr;
	//	auto p_function = ddk::util::DynImport::Instance().get_proc_address("PsGetCurrentProcessId");
	//	RtlPcToFileHeader(p_function, &imageBase);
	//	if (imageBase)
	//	{
	//		DBG_PRINT("ntos image base=%p\r\n", imageBase);
	//		pdbfilename = "";
	//		symname = "";
	//		auto pdbget=ddk::util::DynImport::Instance().getPdbInfo(imageBase, pdbfilename, symname);
	//		if (pdbget)
	//		{
	//			DBG_PRINT("load pdb filename:%s/%s\r\n", symname.c_str(), pdbfilename.c_str());
	//			if (http.open(ddk::MS_SYMBOL_SERVER, ddk::MS_SYMBOL_SERVER_USER_AGENT))
	//			{
	//				cabfilename = pdbfilename;
	//				auto index = cabfilename.find(".pdb");
	//				cabfilename.replace(index,index+strlen(".pdb"), ".pd_");
	//				DBG_PRINT("cabfile:%s\r\n", cabfilename.c_str());
	//				std::string uri = "";
	//				uri = ddk::MS_SYMBOL_SERVER_DOWNLOAD_URI;
	//				uri += "/";
	//				uri += pdbfilename;
	//				uri += "/";
	//				uri += symname;
	//				uri += "/";
	//				uri += cabfilename;
	//				DBG_PRINT("http down name = %s\r\n", cabfilename.c_str());
	//				if (http.get(uri, response))
	//				{
	//					DBG_PRINT("cabfile size = %d\r\n", response.getContentSize());
	//					ddk::CNtFile file;
	//					size_t write = 0;
	//					//CABFile??
	//					file.create(L"\\??\\C:\\1.cab");
	//					file.write(response.getContent(), response.getContentSize(), write);
	//					file.close();
	//				}
	//			}
	//		}
	...
	//KeBugCheck(0x111);
	//ddk::special_data::getInstance().dump_kdblock_info();
	//ddk::nt_patchguard::getInstance().search_pg_context(RegistryPath);
	/*
	if (ddk::nt_patchguard::getInstance().disable_pg(RegistryPath))
	{
		DBG_PRINT("Patchguard Disable OK\r\n");
	}
	*/
...
void test_util_free()
{
	//terminate_tdi_filter();
	//hook_disk.unhook();
	//while (g_ClientsCount)
	//{
	//	ddk::util::sleep(ddk::util::time::milli_seconds(100));
	//}
