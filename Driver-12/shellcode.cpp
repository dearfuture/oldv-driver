...
void inject_dll(DWORD pid, LPCTSTR lpszFileName)
{
	HANDLE hFile = CreateFile(lpszFileName, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, NULL, 0);
	...
	auto total_size = SCSize + dwSize + 4;
	auto p_mem = reinterpret_cast<char *>(VirtualAlloc(NULL, total_size, MEM_COMMIT, PAGE_EXECUTE_READWRITE));
	if (p_mem)
	{
		DWORD dwFlag = FIND_FLAG;
		RtlCopyMemory(p_mem, payload, SCSize);
		RtlCopyMemory(p_mem + SCSize, &dwFlag, 4);
		RtlCopyMemory(p_mem + SCSize + 4, lpContext, dwSize);
		auto h_process = OpenProcess(PROCESS_ALL_ACCESS, FALSE, pid);
		if (h_process &&h_process != INVALID_HANDLE_VALUE)
		{
			auto p_shellcode = VirtualAllocEx(h_process, NULL, total_size + 0x1000, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
			if (p_shellcode)
			{
				DWORD dwRet = 0;
				WriteProcessMemory(h_process, p_shellcode, p_mem, total_size, &dwRet);
				CreateRemoteThread(h_process, NULL, 0, reinterpret_cast<LPTHREAD_START_ROUTINE>(p_shellcode), NULL, 0, NULL);
			}
			CloseHandle(h_process);
		}
		VirtualFree(p_mem, 0, MEM_RELEASE);
	}
